...
(defun VM-add (nomVM src dest)
  (VM-set-registre nomVM dest (+ (VM-get-registre nomVM src) (VM-get-registre nomVM dest)))
  (values))

;; Soustraction
(defun VM-sub (nomVM src dest)
  (VM-set-registre nomVM dest (- (VM-get-registre nomVM dest) (VM-get-registre nomVM src)))
  (values))
...
(defun VM-div (nomVM src dest)
  (let ((src-value (VM-get-registre nomVM src))
    ...
    (if (/= dest-value 0)
        (VM-set-registre nomVM dest (/ src-value dest-value))
        (error "Division par zéro"))
    (values)))

;; Incrément
(defun VM-incr (nomVM dest)
  (VM-set-registre nomVM dest (1+ (VM-get-registre nomVM dest)))
  (values))
